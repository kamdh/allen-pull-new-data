...
import friday_harbor.annotation as Annotation
...
nonzero_ind_mask = Mask(np.where(ann != 0))
nonzero_ind_mask.write_to_hdf5(os.path.join(data_dir,'nonzero_ind_mask.hdf5'))
 
# Save off the left hemisphere mask:
left_hemisphere_mask = Mask(ann.left_hemisphere())
left_hemisphere_mask.write_to_hdf5(os.path.join(data_dir,'left_hemisphere_mask.hdf5'))
 
# Save off the right hemisphere mask:
right_hemisphere_mask = Mask(ann.right_hemisphere())
right_hemisphere_mask.write_to_hdf5(os.path.join(data_dir,'right_hemisphere_mask.hdf5'))
...
left_hemisphere_nonzero_ind_mask.write_to_hdf5(os.path.join(data_dir,'left_hemisphere_nonzero_ind_mask.hdf5'))
...
right_hemisphere_nonzero_ind_mask.write_to_hdf5(os.path.join(data_dir,'right_hemisphere_nonzero_ind_mask.hdf5'))
 
# Create universe mask:
universal_mask = Mask(np.where(ann != np.nan))
universal_mask.write_to_hdf5(os.path.join(data_dir,'universal_mask.hdf5'))
 
# Create structure masks:
f = h5py.File(os.path.join(data_dir,'structure_masks.hdf5'),'w')
ont=Ontology(data_dir=data_dir)
for s in ont.structure_list:
    # Whole region:
    child_mask_list = [Mask(np.where(ann == s_child.structure_id))
                       for s_child in s.child_list]
...
f_inj = h5py.File(os.path.join(data_dir,'injection_masks.hdf5'), 'w')
...
for e in ex_list:
    if os.path.isfile(e.data_file_name):
        ...
        print "%s does not exist" % e.data_file_name
f_inj.close()

# shell creation is now done later
# # Create injection masks with a shell:
# f_inj = h5py.File(os.path.join(data_dir,'/injection_masks_shell.hdf5', 'w')
# for e in ex_list:
